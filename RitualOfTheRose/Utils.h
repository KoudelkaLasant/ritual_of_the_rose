...
public:
	...
	public:
		...
		bool isSteppedNear(pair<float, float> position) {
			...
			List<mapFloor::triangle> toCheck = mapObject::resize(triangles,1.2, positionOnMap);
...
mapFloor::triangle({{54.376184940338135,67.48740673065186}, {55.59239983558655,67.48740673065186}, {55.59239983558655,75.5698561668396}}),mapFloor::triangle({{54.376184940338135,67.48740673065186}, {54.376184940338135,66.72072410583496}, {55.53506016731262,66.72072410583496}}),}),true,Map<string, string>({pair<string, string>({"audio", "1"}), pair<string, string>({"audio source", "CHAPELSTEP"})})),
		...
		maps["ChapelRight1"] = mapInstance("ChapelRight1", CHAPELRIGHT_1, { 3,43 }, List<mapObject>({
			...
			mapObject("Lamp7", false, true, false, imageLookup.getSequenceAsString("ChapelFloorLamp1","STAND_FRONT"),"1",90,imageLookup.layerDefaults["PLAYER"],"1.0","1.0","CENTRE",{10, 11.5}, false, {}, {}),
			mapObject("Lamp8", false, true, false, imageLookup.getSequenceAsString("ChapelFloorLamp1","STAND_FRONT"),"1",90,imageLookup.layerDefaults["PLAYER"],"1.0","1.0","CENTRE",{17, 11.5}, false, {}, {}),
...
mapFloor::triangle({{31.092330813407898,13.46537321805954}, {28.89198064804077,13.46537321805954}, {28.89198064804077,15.055710077285767}}),}), List<pair<string,string>>({
			...
			mapFloor("ChapelFloor", List<mapFloor::triangle>({mapFloor::triangle({{99.53176975250244,0.22029876708984375}, {0.257430924102664,57.73525834083557}, {99.53176379203796,57.73526430130005}}),mapFloor::triangle({{99.53176975250244,0.22029876708984375}, {-0.3891110420227051,-0.0902902742382139}, {0.257430924102664,57.73525834083557}}),}),true,Map<string, string>({pair<string, string>({"audio", "1"}), pair<string, string>({"audio source", "CHAPELSTEP"})})),
			}), {}, { 5000, 5000 }, Map<string, string>());
}
	...
	pair<float, float> moveLHSCloserToRHS(pair<float, float> LHS, pair<float, float> RHS, bool ignoreFirst, bool ignoreSecond, float unit) {
		bool firstAreSame = abs(LHS.first - RHS.first) < 0.05;
		bool secondAreSame = abs(LHS.second - RHS.second) < 0.05;
